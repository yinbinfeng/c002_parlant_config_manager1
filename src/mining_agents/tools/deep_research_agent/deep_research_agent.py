...
class DeepResearchAgent(ReActAgent):
    ...
    def _extract_text_from_block(block: Any) -> str:
        """兼容 dict / TextBlock 对象，安全提取 text 字段。"""
        if isinstance(block, dict):
            return str(block.get("text", ""))
        return str(getattr(block, "text", ""))

    @classmethod
    def _extract_text_from_blocks(cls, blocks: Any) -> str:
        """从 block 列表中提取首个文本块内容。"""
        if isinstance(blocks, list) and blocks:
            return cls._extract_text_from_block(blocks[0])
        return cls._extract_text_from_block(blocks)

    @staticmethod
    def _preview_text(text: Any, limit: int = 1200) -> str:
        s = str(text or "")
    ...
    async def reply(
        ...
        await self.memory.add(current_msg)
    ...
    async def _acting(self, tool_call: ToolUseBlock) -> Msg | None:
        ...
        intermediate_report = ""
        chunk = ""
        search_tool_stream_finished = False
        try:
                    ...
                    await self.memory.add(
                        Msg(
                            ...
                            [
                                TextBlock(
                                    type="text",
                                    text=self._extract_text_from_blocks(
                                        chunk.content,
                                    ),
                                ),
                            ],
            ...
            if tool_call["name"] == self.search_function:
                search_tool_stream_finished = True
            return None

        except Exception as e:
            loguru_logger.error(
                "DeepResearchAgent 工具调用失败 name={} id={} input={!r}: {}: {}",
                tool_call.get("name"),
                tool_call.get("id"),
                tool_call.get("input"),
                type(e).__name__,
            ...
            loguru_logger.error(
                "DeepResearchAgent 工具调用 traceback:\n{}",
            ...
            if tool_call["name"] == self.search_function and search_tool_stream_finished:
    ...
    async def get_model_output(
        ...
        try:
            logger.info(
                "DeepResearchAgent model input msgs(count=%s): %s",
                len(msgs),
                self._preview_text(
                    json.dumps(
                        [{"role": getattr(m, "role", None), "content": getattr(m, "content", None)} for m in msgs],
                        ensure_ascii=False,
                    ),
                ),
            )
        except Exception:
            logger.info("DeepResearchAgent model input msgs(count=%s)", len(msgs))
        if format_template:
            ...
            logger.info(
                "DeepResearchAgent model output(structured): %s",
                self._preview_text(blocks),
            )
            ...
            if stream:
                ...
                blocks = res.content
            logger.info(
                "DeepResearchAgent model output(text): %s",
                self._preview_text(blocks),
            )
            return blocks
    ...
    async def summarize_intermediate_results(self) -> ToolResponse:
        ...
        intermediate_report = self._extract_text_from_blocks(blocks)
    ...
    async def _generate_deepresearch_report(
        ...
        if self.report_index > 1:
            ...
            for index in range(self.report_index):
                ...
                draft_output = read_draft_tool_res_msg.content[0]["output"]
                inprocess_report += (
                    self._extract_text_from_blocks(draft_output)
                    + "\n"
                )
        ...
        final_report_content = self._extract_text_from_blocks(blocks)
        logger.info(
