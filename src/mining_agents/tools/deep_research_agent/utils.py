...
def _safe_encode_text(text: str) -> str:
    """统一清理不可见字符，保留原始 Unicode 文本内容。"""
    if not text:
        return text
    # 仅移除常见零宽/BOM 控制字符，避免污染模型上下文。
    return (
        text.replace("\ufeff", "")
        .replace("\u200b", "")
        .replace("\u200c", "")
...
def truncate_by_words(
    ...
    sentence = _safe_encode_text(sentence)
    words = re.findall(
        r"\w+|[^\w\s]",
        sentence,
        re.UNICODE,
    ...
    for i, word in enumerate(result):
        if i == 0:
            ...
            truncated_sentence += word
    return _safe_encode_text(truncated_sentence)
