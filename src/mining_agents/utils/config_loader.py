...
class ConfigLoader:
    ...
    def _normalize_agent_base_name(self, agent_name: str) -> str:
        """归一化动态实例名到基础 Agent 名称。

        例如：
        - ProcessAgent_node_000 -> ProcessAgent
        - GlossaryAgent_node_12 -> GlossaryAgent
        """
        if not agent_name:
            return agent_name
        return re.sub(r"_node_\d+$", "", agent_name, flags=re.IGNORECASE)
    ...
    def load_agent_config(self, agent_name: str) -> Dict[str, Any]:
        ...
        Returns:
            代理配置字典
        """
        base_name = self._normalize_agent_base_name(agent_name)
        normalized = self._to_snake_case(agent_name)
        base_normalized = self._to_snake_case(base_name)
        candidate_names = [
            f"{agent_name.lower()}.yaml",
            f"{normalized}.yaml",
            f"{normalized}_agent.yaml",
            f"{base_name.lower()}.yaml",
            f"{base_normalized}.yaml",
            f"{base_normalized}_agent.yaml",
        ]
        config_file = None
