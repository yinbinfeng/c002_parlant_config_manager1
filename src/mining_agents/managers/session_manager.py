...
class SessionManager:
        ...
        payload = {
            ...
            "progress": {
                ...
                "step_details": {
                    ...
                    "step6": {"status": "pending", "percent": 0},
                    "step7": {"status": "pending", "percent": 0},
                    "step8": {"status": "pending", "percent": 0},
                },
            },
            "output": {
    ...
    def submit_clarification(self, *, session_id: str) -> bool:
        with self._lock(session_id):
            ...
            t = threading.Thread(target=self._run_steps2to8_background, args=(session_id, True), daemon=True)
            t.start()
            return True
    ...
    def skip_clarification(self, *, session_id: str) -> bool:
        with self._lock(session_id):
            ...
            t = threading.Thread(target=self._run_steps2to8_background, args=(session_id, False), daemon=True)
            t.start()
            return True
    ...
    def _run_steps2to8_background(self, session_id: str, use_clarification: bool) -> None:
        try:
            paths = self._paths(session_id)
            sess = self._read_session(session_id)
            business_desc = (sess.get("input") or {}).get("description", "")
            ...
            async def _run():
                ...
                await step_manager.run_steps(start_step=2, end_step=8, context=ctx)
                return ctx
            ...
            with self._lock(session_id):
                ...
                sess["current_step"] = 3
                sess["updated_at"] = _now_iso()
                sess["progress"]["current_action"] = "Step2-8：生成配置中"
            ...
            step8_dir = step_manager.get_step_output_dir(8)
            step8_result = step8_dir / "result.json"
            step8_report = step8_dir / "step8_validation_report.md"
            step8_cert = step8_dir / "step8_compliance_certificate.json"
            parlant_root = None
            ...
            if step5_result.exists():
                try:
                    ...
                    parlant_root = None
            if step8_result.exists():
                try:
                    s8 = json.loads(step8_result.read_text(encoding="utf-8"))
                    validation_summary = s8.get("metadata", {})
                except Exception:
                    validation_summary = None
            ...
            with self._lock(session_id):
                ...
                for k in ("step2", "step3", "step4", "step5", "step6", "step7", "step8"):
                    sess["progress"]["step_details"][k] = {"status": "completed", "percent": 100}
                sess.setdefault("output", {})["config_package_path"] = parlant_root
                sess.setdefault("output", {})["validation"] = validation_summary
                sess.setdefault("output", {})["validation_report_path"] = (
                    str(step8_report) if step8_report.exists() else None
                )
                sess.setdefault("output", {})["compliance_certificate_path"] = (
                    str(step8_cert) if step8_cert.exists() else None
                )
                self._write_session(session_id, sess)
