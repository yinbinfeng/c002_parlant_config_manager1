...
class AgentOrchestrator:
    ...
    def __init__(self, config: dict):
        ...
        self.agent_module_map = {
            ...
            "UserProfileAgent": "user_profile_agent",
            "ConfigAssemblerAgent": "config_assembler_agent",
            "ComplianceCheckAgent": "compliance_check_agent",
        }
    ...
    async def execute_agent(
        ...
        if sn is not None:
            try:
                self.performance_tracker.record_step_agent_call(int(sn), agent_name)
            except (TypeError, ValueError):
                pass

        self.logger.info(f"Executing agent '{agent_name}' with task: {task[:50]}...")
        
        self.performance_tracker.start_agent(agent_name)
        ...
        try:
            ...
            try:
                ...
                err_text = repr(e)
            self.logger.error(f"Agent '{agent_name}' execution failed: {err_text}")
            raise
        finally:
            self.performance_tracker.end_agent(agent_name)
    ...
    async def execute_coroutines_limited(
        self,
        coroutines: List[Any],
        *,
        max_parallel: int,
    ) -> List[Any]:
        """有限并发执行协程列表（用于 Step5/Step6 等高并发阶段的并发上限控制）。

        说明：
        - 迁移到 v2（8 步法）时，Step5 会出现“节点间并发”的任务组，此方法提供统一的限流入口。
        - 不在这里做重试与错误降级；由上层 step handler 结合系统配置实现。
        """
        ...
        wrapped = [_run_one(c) for c in coroutines]
