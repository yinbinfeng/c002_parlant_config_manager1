...
def _extract_json_array_string_from_llm_response(raw: str) -> str:
    """从 LLM 回复中抽出问题 JSON 数组文本（thinking + ```json 代码块 / 纯围栏）。"""
    s = (raw or "").strip()
    if not s:
        return s
    lower = s.lower()
    ...
    if s.lstrip().startswith("```"):
        ...
        if lines and lines[-1].strip() == "```":
            lines = lines[:-1]
        return "\n".join(lines).strip()
    # 少数网关把 content 序列化成「Python 列表字面量」字符串
    if s.startswith("[") and isinstance(s, str) and ("'type'" in s or '"type"' in s):
        try:
            ...
            if isinstance(arr, list):
                ...
                inner = "".join(text_parts).strip()
                if inner:
                    return _extract_json_array_string_from_llm_response(inner)
...
class RequirementAnalystAgent(BaseAgent):
    ...
    def __init__(self, name: str, orchestrator, **kwargs):
        ...
        use_react = bool((self.model_config or {}).get("use_react", False))
        if use_react:
            # 按配置启用 ReAct（默认关闭，Step1 走 JSON-only LLM 路径）。
            self._init_react_agent()
        else:
            self.logger.info("RequirementAnalyst ReAct disabled by config (use_react=false)")
    ...
    async def _generate_questions_with_llm(self, business_desc: str, context: Dict) -> List[Dict[str, str]]:
        ...
        response = _extract_json_array_string_from_llm_response(response)
        parsed = None
        try:
