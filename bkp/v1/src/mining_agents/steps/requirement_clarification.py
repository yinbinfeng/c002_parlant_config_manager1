...
async def requirement_clarification_handler(context, orchestrator):
    ...
    result = await orchestrator.execute_agent(
        ...
        context={
            ...
            "output_dir": context.get("output_dir", "./output/requirement_clarification"),
        }
    )
    ...
    if bool(context.get("skip_clarification", False)):
        ...
        use_clarification = False
        feedback_text = "通过 --skip-clarification 参数跳过人工澄清，继续执行后续步骤。"
        ...
        if not sys.stdin.isatty():
            ...
            logger.info("检测到非交互式环境，默认选择 yes 继续执行。")
        else:
